#!/usr/bin/env node
// Publishes Diplomacy players' submitted moves at the deadline.
//
// Players submit orders as gist comments (one comment each, edited in place —
// see js/publish.js). This script, run hourly by the scheduled GitHub Action,
// copies the valid submission of every assigned power into that power's
// moves-<power>.json file in the game gist — but only once the game's
// deadline has passed (game.json `deadline`, an ISO timestamp the game master
// confirms in the app each phase). No deadline set = nothing auto-publishes.
// A power already published for the current phase is skipped, so comment
// edits after the deadline change nothing; the game master can un-publish
// from the app to allow a correction.
//
// Environment:
...
async function processGame(gistId, description) {
  ...
  // before it) this game is left alone
  if (!IGNORE_DEADLINE) {
  ...
  for (const [power, login] of Object.entries(players)) {
    ...
    const sub = findSubmission(comments, login);
