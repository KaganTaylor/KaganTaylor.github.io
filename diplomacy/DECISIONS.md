...
Everyone else opening the link gets a live view — write and drag orders, submit them if assigned — but their Resolve is relabelled **👁 Preview result** and adjudicates on a *throwaway clone* of the position (`shadowGame()` / `previewResolve()` in `js/app.js`). The playback panel reads nothing but the history entry it is handed, so the whole step-through, the animation and 📋 Copy results all work unchanged while the stored game is never touched; closing the playback re-renders the live position over the top.

The old behaviour resolved the viewer's own copy, and only after an auto-publish deadline had passed. That had it backwards in both directions. It let a stray click silently walk a player's board a turn ahead of the table's — the divergence that makes someone plan against a position nobody else can see — while *withholding* the harmless and genuinely useful half. Because previewing costs nothing now, it is offered at all times: guess what the other six will do, preview it, and 🌿 **Branch to analysis** if the outcome was worth keeping.

A preview keeps `Continue ➜` (relabelled *▶ Play the moves*) so the animation still plays, but it stops on the resulting position instead of advancing a phase — there is no next phase to advance into. **Replaying a past phase** (History panel → **Replay**) is the same shape for the same reason and now gets the same treatment: `startPlayback()` used to hide `Continue ➜` whenever a playback was read-only, on the theory that read-only meant "nothing to advance" — true for a preview, but a plain replay is *also* read-only and has nothing to advance into either, so the same flag left it stranded with no way to watch the moves execute, only `Back to current turn`. The fix reads what actually decides the outcome — `playback.readonly`, not `playback.preview` — so both land on the final position instead of calling `endPlayback()`, and only a genuine resolve (live, or `catchUpNext()`/`resolveRevealedLocally()` stepping through, neither of which is read-only) still advances the game when its animation finishes.

That leaves **🌿 Analysis** as the escape hatch from every read-only situation (below). A preview's own **🌿 Branch to analysis** (`pb-branch`) and the topbar's **🧪 Copy to sandbox** (reachable from ⚙ Settings, and from a preview too) are no longer the same idea wearing two labels: branching re-resolves the previewed orders for real inside a brand-new line rooted at the live position — an ordinary line from there, cleared the moment the live game moves on — while copying to sandbox makes the permanent, independent kind of copy analysis deliberately isn't. A sandbox copy records where it came from (`branchedFrom`), shows it, and offers **↩ Open source game** to get back.

### A spectator's order box is theory-crafting, not a submission

A spectator (no assignment, not the GM) used to get a country picker in the Orders panel — "👁 View all countries" or "Play as X" — so they could isolate one power's block in the textarea, plus a "✎ Private scratch pad" note explaining that none of it reaches the published game. Both are gone: the picker (`country-row`/`country-select`, `renderCountrySelect()`) was removed outright rather than just hidden, and `renderDraftNote()` now only ever shows its note for the GM.

The box itself, and 👁 Preview result / ⏭ Preview to final, stay exactly as above — a spectator can still load the game, write orders for any or all powers in the merged all-countries view, and preview the outcome, then **← Back to the live position** to drop it. What's gone is the *bookkeeping around* that: nothing narrows the box to one power's lines, and nothing needs to explain that it's a scratch pad since there is no longer a control that could make it look like anything else. `myCountry()` still exists as a permission primitive (`roles.js`) — a player's is still set by their assignment — a spectator's is simply never set now, so `myCountry()` returns `''` for them unconditionally and the box always shows every power merged together.

For the same reason ⬇ **Load submitted moves** (see above) no longer shows for a spectator at all: it would silently overwrite whatever they were theory-crafting with the table's actual moves, which is the opposite of what the box is for once there is no picker to isolate a throwaway single-power draft from.
...
**"The line's own starting phase" is `lineStartIndex()`, not index 0.** A branched line inherits its source's history up to the cut point (so Undo and the History dropdown work from the moment it exists), which means a line cut two phases in *begins* at index 2 of its own history. The comparison in `branchParent()` is against that, and app.js asks `branchParent()` rather than re-deriving the rule; when it compared against 0, every branch off a nested line came out a child of it whatever phase it was taken at, and the panel said "nested" to match. The one other place that must count from the root rather than from the cut is ↥ *Use these orders live*: those orders are only for the live turn while the line stands at the root (`history.length === 0`), and `ownPhaseCount()` — phases the line played itself — is the wrong question there.
...
For the same reason the GM's copy of ⬇ **Load submitted moves** (`btn-load-moves`, the Orders panel's online row) is gone — `loadMovesBtn.hidden = isOwnerView() || !assignedPower()`. It could only ever appear *after* ⬇ Load orders had already run (the Orders panel is gated on `gmOrdersLoaded`), so it showed the GM what they had just loaded, and in manual mode nothing is published yet, so its only possible answer was "no published moves for this phase". Two buttons for one act, disagreeing about it. Its one remaining meaning is a player reloading their own submission; a spectator no longer sees it at all — theirs is a theory-crafting box (see *A spectator's order box is theory-crafting, not a submission* below) that the table's moves would otherwise clobber. That path clears `game.deadline` on publish — a stale "already passed" timestamp must never carry over and silently auto-publish the *next* phase off an empty box; the GM (or auto mode, next time someone submits) confirms a fresh deadline every phase. The repo's GitHub Action remains a manually-dispatched convenience for copying reveals into the per-power files as a durable record.
...
**Anything that gathers the table's orders refreshes first.** Nothing polls the network on a timer — the 60s tick re-renders from `online.comments` as last fetched — so that cache is as old as the last explicit refresh, and players submit in the last minutes before a deadline. `autoPublishIfDue()`, `resolveRevealedLocally()` and the GM's ⬇ Load orders (`gmLoadOrders()`) therefore each `await refreshOnlineStatus()` before reading a single comment. Two of the three used to skip it (the GM's only refreshed when it had *no* comments at all), and the failure is silent in the worst way: a power's on-time orders simply absent from the box that then gets resolved and published. `gmLoadOrders()` also refuses to proceed when the refresh came back with nothing, since a box of blank templates because GitHub was unreachable is indistinguishable from a phase nobody submitted for.
...
**A viewer's board follows the gist forwards by stepping, backwards by reloading — unless the viewer is a spectator, who just always sees the current board.** A player's stepping exists to show them how *their own submission* turned out — dropping them onto a new position without showing them how it got there is the thing catch-up exists to prevent. A spectator has no submission riding on the outcome, so there is nothing for the step-through to be showing them; it was only ever friction, one click standing between them and a board they have no stake in changing. `syncViewerToGist()` therefore branches on role right after the "nothing changed" check: a spectator (`R.gameMode(g) === 'spectator'`) goes straight to `adoptPublishedPosition()`, silently, whether the gist moved forward or the GM undid/edited it — both look the same from a spectator's seat, a board that is now here. `renderCatchUpButton()` hides ▶ Resolve new orders! outright for a spectator for the same reason, and auto mode's optimistic reveal (`localAutoResolveAvailable()`/`resolveRevealedLocally()`, below) gets a silent twin, `resolveRevealedLocallySilently()`, called from `refreshOnlineStatus()` instead of waiting on a click. A returning viewer opening a game from the home screen goes through `loadPublishedGame()`, not `refreshOnlineStatus()` — it used to reimplement catch-up detection inline, and with the simpler, length-only check (`fresh.history.length > local.history.length`) that the next paragraph describes replacing in `syncViewerToGist()` for exactly this reason: an undone or edited board came back undetected here too, even after that fix landed elsewhere, since this call site never went through it. It now just calls `syncViewerToGist()` with the fetch it already has, so there is exactly one place this rule lives, for every role.

A player (assigned, or the game master 🎭 Playing as their own power) keeps the three-way split that used to apply to every viewer. When the gist has phases a viewer hasn't seen *on top of the ones they have*, they are flagged for ▶ Resolve new orders! (`catchUpTarget`) so each one plays out. But when the gist's position is not one the viewer's history leads to — the GM undid a phase, or corrected the board in ✏ Edit board — there is nothing to step *through*: their copy is of a position that no longer exists. `syncViewerToGist()` takes the gist's position in that case and toasts to say why. The check compares the *position* (`viewerPosition()` — year/season/step/units/scOwners/pending/history), not the history length: an earlier length-only test could see forward motion and nothing else, so an undone phase left the viewer parked on it indefinitely, still able to replay orders the GM had since retracted, while a board edit — which never touches history at all — was invisible. `redoStack` is deliberately excluded from the comparison: it's the GM's private undo bookkeeping, and a viewer who caught up has cleared their own while the gist may still carry the GM's, which is not a divergence. Two copies are never taken this way: a browser holding a `provisionalPhase` (auto mode's optimistic local resolve) is *meant* to sit ahead of the gist, and `reconcileProvisionalPhase()` already owns that comparison; and the GM's own copy is never overwritten even while 🎭 Playing as their own power, since it may hold unpublished work — ⟲ Revert to published is their door.

**That comparison compares facts, not bytes.** `game.json` carries history in a packed form (`js/history-codec.js`: chain-dropped `*Before` snapshots, default-valued order fields stripped, empty tails omitted), and a history resolved in this browser and one decoded off the gist hold the same facts in different shapes — different key order, and empties present in one and absent in the other. Comparing them with a raw `JSON.stringify` called *every* viewer's board changed on their first poll after a phase had been resolved: the toast fired, the step-through was skipped, and `adoptPublishedPosition()` overwrote their history. Two rules came out of it. The wire format is decoded in **one place** (`decodeGameJson()` in `js/publish.js`, behind both `fetchPublished()` and `readGameFile()`), so no reader can hand back the packed shape — `readGameFile()` did, and `refreshOnlineStatus()` never noticed because the packed shape still parses. And `viewerPosition()`/`extendsOurHistory()` reduce each history entry to a **fingerprint** of what makes it that phase — year/season/step, the orders text, and the board it produced — through a key-sorted stringify (`stableStringify`), so the codec is free to change how an entry is stored without any comparison in the app noticing. `test/publish-wire.test.js` pins the two readers to the writer, across a history that spans movement, retreat and winter entries.
...
`isPlayingAsPlayer()` (`js/app.js`) is the one predicate this depends on: `game.isOwner && game.assignedPower && game.playAs === 'player'`. `isReadOnly()` and `isOwnerView()` both fold it in, so once it's true the GM's browser runs through the *exact same code paths* a real player's browser does — private draft orders, a real `📤 Submit moves` that posts a real gist comment under their own login, deadline enforcement, preview-only resolve. Switching back to `👑 Game Master` never touches a submission, because the submission they made while playing was always a genuine one — but it does not simply flip `game.playAs` back and stop, either: see the `provisionalPhase` rollback below, and the order-box handling next.

`setPlayAs()` also carries the order box across the switch itself (`liveDraft`, the same mechanism `enterAnalysis()`/`exitAnalysis()` use to park a draft): the box is re-rendered for the new hat's `myCountry()` filter, but nothing typed or loaded is thrown away by the switch. It used to be — `refreshAll()`'s own `prefillOrders()` resets the box to the blank per-phase template on every call unless told to preserve, which every other "which power's orders am I looking at" change (the spectator country picker) already knew to ask for. `setPlayAs()` didn't, so a GM who had loaded submissions to review, or a player mid-draft, lost the box on a round trip through the other hat.

**Returning to Game Master rolls back an unconfirmed `▶ Resolve new orders!`.** That optimistic local resolve (see *A deadline belongs to a phase, not to a clock* below) advances the real `liveGame` object from only the on-time submission comments and marks it `provisionalPhase` — nobody has reviewed it, and it was never meant to be anything but a preview for the player. Without an explicit rollback, `boardDirty()` cannot tell that board apart from a normal GM edit: it just sees the local position has moved on from `publishedState` and lights up `☁ Publish changes`, which would push that unreviewed resolution live with one click, skipping `gmPublishPreview()`'s load/resolve/review step entirely. `setPlayAs()` therefore undoes the provisional phase (mirroring the divergent branch of `reconcileProvisionalPhase()`) before it ever reaches `boardDirty()`, whenever the switch is back to Game Master. Publishing that phase for real always goes through the normal flow afterward — nothing here shortens it, whether the GM's own preview and the eventual publish would have agreed or not.
...
The one thing that genuinely *is* suspended while playing as a player is the GM's own *authoritative* auto-publish — not the reveal itself, which every other viewer still gets on schedule regardless of what the GM's browser is doing. See "A deadline belongs to a phase, not to a clock" below for why running the game and playing in it cannot both be live in one browser, and its "Known limitation" subsection for exactly what does and doesn't stay visible while that's the case.
...
A game master who leaves their browser in 🧑 Player mode gets no *authoritative* auto-publish at all. The deadline passes, their own ▶ Resolve new orders! shows them the outcome locally, and the gist simply stays where it is until they switch to 👑 Game Master, whose next tick publishes it. Nothing is lost or corrupted — the deadline and its `deadlineFor` stamp stay put — but a GM who assigned themselves a power, which is the common case, has to remember to change hats. That is a worse deal than they had before, traded for never publishing a turn nobody ordered. **This part of the limitation is still open.**

**This is a gap in the authoritative record only — it is not visible to anyone as a delay.** `resolveRevealedLocally()`/`localAutoResolveAvailable()` are available to *every* read-only viewer, not just the GM's own browser, the instant the deadline passes in auto mode (`js/app.js`, guarded on `isReadOnly()` rather than anything about the GM). Every player's own browser reveals the phase for itself independently of what the GM's browser is doing or showing — that reveal was never routed through the GM in the first place, since it's each viewer resolving the same public on-time comments locally, not something the GM's publish hands out. So nobody is kept waiting on the GM to *see* the outcome; what stays missing while the GM sits in Player view is only the gist's own durable copy of it (`game.json`), which matters for a new viewer's first load and for the eventual `reconcileProvisionalPhase()` reconciliation, not for whether current players can already see the board. Don't mistake this limitation for players being blocked from viewing results — that would be a different and worse bug, and it is not this one.

What is no longer available — and never should have looked available — is pushing that local preview live by hand. It used to be possible to switch to 👑 Game Master and press ☁ Publish changes, which published the optimistic resolve exactly as `resolveRevealedLocally()` had computed it: from only the on-time comments, with nobody having reviewed it, entirely bypassing `gmPublishPreview()`'s load/resolve/review step. `setPlayAs()` now rolls the provisional phase back (`undoLastPhase()`, mirroring `reconcileProvisionalPhase()`'s divergent branch) the moment the switch lands on Game Master, before `boardDirty()` can mistake it for a normal GM edit — see *🎭 Play as* above. Publishing that phase now always means going through the real flow: ⏰ Deadline → ⬇ Load orders → Resolve → 📣 Publish results.

The right fix for the remaining gap is not to re-widen the gate back to raw `isOwner` — that is exactly the collision described above. It is to make the authoritative publish independent of which view the browser is showing, so it can run from Player view without the optimistic path being able to feed it a board it has already advanced. Roughly, in rough order of appeal:
