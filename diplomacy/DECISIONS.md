...
That is the whole mechanism. Each player's comment — their **mailbox** — is created with `MAILBOX_BODY`, a placeholder holding no orders, and every submission thereafter is a `PATCH`. `submitOrders()` cannot create a comment at all: it PATCHes, and if there is somehow no mailbox it throws `NO_MAILBOX` rather than posting. Nothing in the app may ever create a comment carrying order text.

The mailbox is created **when the player opens the game**, not when they first submit — `ensureMyMailbox()` on the `refreshOnlineStatus` path. Both would keep orders out of the mail, but creating it at submit time would still fire the one "X commented" notification at the moment they submitted, leaking the timing. Created on load, that single notification lands hours or days earlier and says nothing. It is also free: `refreshOnlineStatus()` already fetches every comment on load and on each 60 s tick, so "do I have a mailbox?" is a lookup in memory, and the POST happens at most once per player per game.
...
#### The email body is rendered late, so the gap is not a guarantee

Everything above assumes the notification GitHub sends on creation reflects the comment *as created*. It does not. **GitHub renders a notification's body when its mailer runs, not when the comment is created** — and that delay is undocumented, unbounded, and not a queue you can wait out (GitHub batches and deduplicates low-priority mail; players report anything from seconds to 15–30 minutes to hours).

The evidence is in one game's own gist. A comment created at `20:40:39` and edited at `20:40:40` — one second — was mailed out **carrying the orders**. In the same gist eleven minutes earlier, a mailbox created at `19:35:09` and first written at `19:38:43` — 3m34s — mailed only the placeholder. Same code, same evening.

So the create/edit split is a mitigation whose coverage cannot be measured, and two things follow:

- **Create the mailbox as early as possible, and make that the only place that creates.** Widening the gap from seconds to minutes-or-days is the whole defence, and it only works if nothing else can create a comment at an awkward moment. `ensureMyMailbox()` is that place; `submitOrders()` lost the ability entirely.
- **Never decide "do I have a mailbox?" from anything but a freshly fetched comment list.** A remembered `localStorage` id (added to make resubmits read-free) was consulted *first* and never verified, so after a player deleted their comment the load-time creator concluded there was nothing to do — and the submit path did the creating, POST and PATCH one second apart. Finding a mailbox in the fetched list is what *records* the id; the id is only ever a PATCH target, never evidence of existence. The freshly created comment is folded into `online.comments` via `rememberWrite()` so a lagging list cannot provoke a duplicate, and there is deliberately no "made one this session" latch — a mailbox deleted mid-session must come back on the next poll.

**And accept that no gap is safe enough**, which is why order text is encrypted rather than timed. A fixed waiting period before a first submission was considered and rejected: it would have imposed a real, visible cost on every player in exchange for a guarantee it could not actually make.
