...
- **The submit path may still create, but only sealed, and only as a last resort.** `createSubmission()` posts the submission itself rather than sending the player away to press the button again. This is safe *because* the orders are sealed: the notification carries ciphertext, and splitting it into an empty POST plus a PATCH would leak precisely as much (that is the whole finding above) for an extra request. With no seal key it does split — never POST cleartext order text — but that path is a stopgap, not a defence.
