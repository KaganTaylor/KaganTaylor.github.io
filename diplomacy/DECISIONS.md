...
That is the whole mechanism. Each player's comment — their **mailbox** — is created with `MAILBOX_BODY`, a placeholder holding no orders, and every submission thereafter is a `PATCH`. `submitOrders()` has no POST-with-content path at all; it resolves a mailbox (creating an empty one first if the player somehow has none) and always edits. Nothing in the app may ever create a comment carrying order text.
...
- **A mailbox must never parse as a submission.** `MAILBOX_BODY` starts with `ORDERS_MARKER` so `findMyComment()` recognises it, but omits `power`/`year`/`season`/`step` so `parseSubmission()` rejects it — an empty mailbox reads as "— waiting", never as a submission of no orders. `findMyComment()` also prefers a real submission over a bare placeholder, and ignores comments without the marker, so an ordinary chat comment on the gist is never overwritten.
...
Finally, **where duplicates already exist, the most recently edited comment wins** — `findSubmission()` and `findMyComment()` both rank that way, and must keep agreeing, or a resubmit would edit one comment while the UI read another. Games played before the fix carry duplicates, and preferring the older one is wrong twice: it shows superseded orders, and it would let a player submit on time, edit a duplicate after the deadline, and be judged by whichever stamp suited them. `tools/publish-moves.js` mirrors the rule.

This closes the *push* channel only. Submissions are still cleartext, so anyone who deliberately opens the gist can read them before the deadline; "don't read the gist comments early" remains a house rule, like not reading someone else's postcard. Closing that too needs client-side encryption — designed in full but not built, see [`proposals/sealed-orders.md`](proposals/sealed-orders.md).
