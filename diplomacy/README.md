...
⚠️ **Don't delete your comment.** It is recreated automatically within a minute, but the mailbox only protects you because it is old: GitHub composes notification emails *when it gets round to it*, not when the comment appears, so a comment created and filled in quickly can still go out complete with your orders.
