...
Practice copies are all just sandboxes. **Publishing a sandbox is what makes it the real game.**
...
8. If you've assigned your own GitHub username to a power in **👥 Set players**, **⚙ Settings → Play as** lets you switch between **👑 Game Master** and your power (shown as its flag and name) — genuinely playing your own power, not a simulation: a private draft order box locked to that power, a real 📤 Submit orders posted under your own account, the same deadline rules as anyone else. Your GM controls (including the order box's load/resolve/publish flow above) hide while playing. Your draft order box comes back with you either way — nothing you had typed or loaded is lost switching hats. Switching back to Game Master **does** discard one thing on purpose: an unconfirmed **▶ Resolve new orders!** preview made while playing (see the note below) is rolled back, so it can never be pushed live by accident — you always publish through the normal load/resolve/publish flow above instead.

   ⚠ **Auto publish is suspended while you're playing your power.** In 🧑 Player view your browser runs the player's code, which includes the player's **▶ Resolve new orders!** — an optimistic local resolve that shows you the outcome but deliberately publishes nothing. Auto publish is the game master's job, so it waits for the 👑 Game Master view rather than running behind that local resolve (which is how a phase nobody had ordered in once got published as an all-hold). Practically: if you play a power in an auto-publish game, switch to 👑 Game Master when the deadline is due — the next check publishes as normal. Nothing is lost either way; the deadline stays put and the phase stays unpublished until you do, and any local preview you saw as the player is discarded on the way back rather than left sitting behind ☁ Publish changes. This is a known limitation of the current design, not the intended end state: publishing without ever entering the GM view is a change we expect to make.
...
5. Whether other powers have submitted yet is not shown to you — only the game master can check that. **🌿 Analysis**, in the topbar, is where you work out what to do: a private tree of what-ifs off the live position, nothing in it reaching the real game until you come back and submit.
6. **History is read-only.** Picking a past phase and pressing **Replay** steps back through it exactly as it happened; the order box disappears entirely while you're looking at it (there's nothing to edit) and returns once you step back to the current turn. Analysis and standings stay available throughout.
...
Runs everything that doesn't need a browser: the full DATC suite (**167/167**), both house-rule suites, and unit suites over the order parser, the order-text buffer, the permission model, the deadline/submission rules, the gist-comment format, the sealing, the analysis tree, and what `stripForPublish` is allowed to put in a public gist. No dependencies and no install step — `package.json` exists only to tell Node that `js/` holds the same ES modules the browser loads directly.
