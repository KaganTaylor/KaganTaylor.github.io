...
3. Pick the **⚙ publish mode**: **manual** (default) — once the deadline passes, you load and resolve the submissions yourself before anyone sees them; **auto** — the game resolves and publishes itself the moment the deadline passes, no action needed. Auto mode needs your browser open on the game, in the 👑 Game Master view, to actually push the result — see the ⚠ note in step 8.
