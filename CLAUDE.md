...
- **`analysis.js`** — the 🌿 analysis tree: **plans** (the focus power's orders — a folder) and **variations** (what the other six do — the only kind that resolves), the position each one starts from and leads to, and `positionKey`/`rootMatches`, the rule deciding when the whole tree is void. The tree lives on `liveGame.analysis`; a line is never a saved game. Read `DECISIONS.md` § *🌿 Analysis* before touching it — the two-level split and the one-root lifetime rule are both load-bearing.
...
- **An analysis line is a view, not a game.** It never enters the saved-games map and never appears on the home screen; the tree is rooted at one position and is discarded when the live game moves past it. That is what stops it going stale, and it is why `🧪 Copy to sandbox` still exists for anything worth keeping.
