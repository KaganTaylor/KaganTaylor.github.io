...
runs everything that does not need a browser — the full DATC suite (target **167/167**), both house-rule suites, and the unit suites over the pure modules (`parser`, `orders-text`, `roles`, `online-rules`, `submission-format`, `seal`, `format`, `analysis`) plus `publish-strip` (what `stripForPublish` may put in a public gist — it asserts the payload's exact key set, so **a new field on the game object fails the suite until it is classified** as shared or private). `diplomacy/package.json` exists only to mark `js/` as ES modules for Node: **no dependencies, no lockfile, no install step, no build.** Do not add any.
...
- **`roles.js`** — the permission model: `isOnline`/`isSandbox`/`isAnalysisLine`/`gameMode`/`isPlayingAsPlayer`/`isReadOnly`/`isOwnerView`/`boardDirty`/`myCountry`/`assignedPower`. Ten pure questions about a game object, and the gate on every control in the app.
...
- **`analysis.js`** — the 🌿 analysis tree: **lines** (each one a whole game object of its own, run as many phases forward as you like) and **folders** (organisation, nothing else), plus `branchParent`/`branchFrom` (the sibling-or-child rule, decided by the phase on screen), `isStale`, `moveNode`/`groupSiblings` for drag-and-drop, and `positionKey`/`rootMatches`, the rule deciding when the whole tree is void. The tree lives on `liveGame.analysis`; a line is never a saved game. Read `DECISIONS.md` § *🌿 Analysis* before touching it — the branch rule and the one-root lifetime rule are both load-bearing.
...
- **`app.js`** — the entry point and everything DOM/UI: event wiring, order-box sync (`syncOrderLine()` is the single funnel from board drags/clicks back into order text — the order text is the source-of-truth representation, never a second parallel order-object model), playback/step-through state, mobile bottom-sheet UI (`applyMobileSheetUI()`), GM vs. read-only-viewer control gating (`isOwner`).
  ...
  **Two pointers.** `liveGame` is the real game — the only thing ever saved under its own name, and the only thing the online/publish/deadline block (one contiguous section near the bottom of the file) touches. `game` is what is *on screen*: normally `liveGame`, but while a 🌿 analysis line is open it points at that line's own game object instead. A line *is* an ordinary game object, which is what lets every drag, resolve, board edit and playback work on one unchanged. When adding code, decide which you mean: rendering and order entry want `game`; anything about publishing, submitting, deadlines or roles wants `liveGame`. `saveCurrent()` is the one save path that knows the difference, and `validateAnalysis()` at the top of `refreshAll()` is the single place the tree's lifetime is enforced.
...
- **Only the owning browser can advance a published game** (`isOwner`, tied to the token that published it); everyone else is read-only and can open a private 🌿 analysis line or 🧪 copy the position into a sandbox.
- **An analysis line is a view, not a game.** It never enters the saved-games map and never appears on the home screen; the tree is rooted at one position and is discarded when the live game moves past it. That is what stops it going stale, and it is why `🧪 Copy to sandbox` still exists for anything worth keeping. A line nonetheless *is* a real game object stored on its node, so resolving/undoing/replaying inside one are the ordinary `state.js` calls on the ordinary object — `game` points at it directly.
- **Where a branch lands is decided by the phase on screen**, not by a menu: at the open line's own starting phase it becomes a sibling, at any later phase a child of that line. `viewedIndex()` in app.js is the input; the panel says which it will be before the click.
- **`stripForPublish` is a drop list.** `game.json` is world-readable, so any new field on the game object is published unless it is explicitly dropped. `test/publish-strip.test.js` fails until it is classified.
